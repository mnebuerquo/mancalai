...
def genMoves(state, movecount=0, chain=[]):
    ...
    for m in new:
        ...
        if player == s.getCurrentPlayer(newstate):
            result.append(genMoves(newstate, chain+[m]))
        else:
...
def alphaBeta(node, alpha, beta, maximizingPlayer, meta, depth, maxdepth):
    ...
    if len(children)==0 or depth>=maxdepth:
        ...
        for move in children:
            cvalue = alphaBeta(child, bestValue, beta, False, meta, depth+1, maxdepth)
        ...
        bestValue = beta
        for move in children:
            cvalue = alphaBeta(child, alpha, bestValue, True, meta, depth+1, maxdepth)
